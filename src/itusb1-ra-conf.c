/* ITUSB1 Rev. A Configuration Program - Version 1.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "Error: Missing argument.\nUsage: itusb1-ra-conf SERIALNUMBER\n");
...
void write_mfg_str1(libusb_device_handle *devhandle)  // Writes the first half of the manufacturing string into the OTP ROM
{
    unsigned char control_buf_out[64] = {
        0x1E, 0x03, 0x42, 0x00, 0x6C, 0x00, 0x6F, 0x00,  // Bloguetrónica (the first byte should be 0x1C - please read note at the bottom)
...
{
    unsigned char control_buf_out[64] = {
        0x30, 0x03, 0x49, 0x00, 0x54, 0x00, 0x55, 0x00,  // ITUSB1 USB Test Switch (the first byte should be 0x2E - please read note at the bottom)
        0x53, 0x00, 0x42, 0x00, 0x31, 0x00, 0x20, 0x00,
...
{
    unsigned char control_buf_out[64] = {
        0x1C, 0x03, 0x49, 0x00, 0x55, 0x00, 0x31, 0x00,  // IU1-A0xxxxxx  (the first byte should be 0x1A - please read note at the bottom)
...
void write_usb_cfg(libusb_device_handle *devhandle)  // Writes the USB configuration values into the OTP ROM
{
    unsigned char control_buf_out[10] = {
        ...
        0x96, 0x8C,  // PID is 0x8C96
        0x28,        // Maximum power is 80mA (0x28)
...
/* Important note:
   The descriptor length should be 2 * length_of_utf16_str + 2. However, the
   CP21xx Customization Utility counts two extra bytes, as if the descriptor
   string was null-terminated. However, this should not be, but the length was
   not rectified in any case (that is, for the manufacturer, product and
   serial strings), in order to maintain backward compatibility with version
   1.0 of ITUSB1 Manager. */
